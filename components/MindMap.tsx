...
export default function MindMap({ data, onNodeClick }: MindMapProps) {
  ...
  useEffect(() => {
    ...
    try {
      mermaid.render('mindmap', data).then(({ svg }) => {
  ...
  const findNodeById = (node: NodeData, id: string): NodeData | undefined => {
    ...
    if (!node.nextSteps) return undefined
    
    for (const child of node.nextSteps) {
  ...
  // if (!data.id || !data.nextSteps?.length) {
  if(!data) {
    return (
